  ...
  startSessionTimer() {
    setInterval(() => {
      if (this.dom.sessionDuration) {
        ...
        this.dom.sessionDuration.textContent = `${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;
      }
    }, 1000);
  }
  ...
  updateVitalSigns(data) {
    ...
    if (this.dom.bpValue) {
      if (
        data.sensors &&
        data.sensors.nibp &&
        typeof data.bp_systolic !== "undefined"
      ) {
    ...
    if (this.dom.tempCoreValue && this.dom.tempSkinValue) {
      if (
        data.sensors &&
        data.sensors.temp &&
        typeof data.temp_cavity !== "undefined"
      ) {
        this.updateVitalCard(
          "tempCore",
          data.temp_cavity,
    ...
    for (let i = 0; i < data.x.length; i++) {
      ...
      if (time >= startTime && time <= currentTime) {
        ...
        const normalizedValue =
          (value - chart.range.min) / (chart.range.max - chart.range.min);
